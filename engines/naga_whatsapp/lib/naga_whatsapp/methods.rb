...
module NagaWhatsapp
  class << self
    ...
    def wa_config(environment = Rails.env)
      whatsapp_numbers_config[environment]
    end

    def send_message(recipient_number, message_text, environment: Rails.env)
    ...
    def send_location(recipient_number, location_name, location_address, latitude, longitude, environment: Rails.env)
      config = wa_config(environment)
      sender_id = config[:phone_number_id]
    ...
    def send_image(recipient_number, imagepath, environment: Rails.env)
      config = wa_config(environment)
      sender_id = config[:phone_number_id]
    ...
    def send_sticker(recipient_number, stickerpath, environment: Rails.env)
      config = wa_config(environment)
      sender_id = config[:phone_number_id]
    ...
    def get_templates(environment: Rails.env)
      config = wa_config(environment)
      business_id = config[:business_id]
      client.templates.list(business_id: business_id).records
    end

    def send_template(recipient_number, language, template_name, parameters, environment: Rails.env)
      config = wa_config(environment)
      sender_id = config[:phone_number_id]
    ...
    def send_template_alert(recipient_number, alert_text, environment: Rails.env)
      send_template(recipient_number, "en", "alert_message", [
        { parameter_name: "notification", type: "text", text: alert_text}
      ], environment: environment)
    end

    def send_template_farmingupdate_my(recipient_number, update_text, environment: Rails.env)
      send_template(recipient_number, "ms", "daily_farming_update_my", [
        { parameter_name: "update", type: "text", text: update_text}
      ], environment: environment)
    end

    def send_template_farmingupdate(recipient_number, update_text, environment: Rails.env)
      send_template(recipient_number, "en", "daily_farming_update", [
        { parameter_name: "update", type: "text", text: update_text}
      ], environment: environment)
    end

    def send_template_helloworld(recipient_number, environment: Rails.env)
      send_template(recipient_number, "en_US", "hello_world", [], environment: environment)
    end

    def update_flow(flow_id, flow_name, flow_data, environment: Rails.env)
      data = {
        name: flow_name,
        categories: [ "OTHER" ],
    ...
    def create_flow(flow_name, flow_data, environment: Rails.env)
      config = wa_config(environment)
      data = {
        name: flow_name,
        categories: [ "OTHER" ],
      ...
      client.send_request(endpoint: "#{config[:business_id]}/flows", params: data, http_method: 'post')
    end

    def publish(flow_id, environment: Rails.env)
      client.send_request(endpoint: "#{flow_id}/publish", http_method: 'post')
    end

    def get_all_flows(environment: Rails.env)
      config = wa_config(environment)
      client.send_request(endpoint: "#{config[:business_id]}/flows", http_method: 'get')
    end

    def get_flow_details(flow_id, environment: Rails.env)
      begin
        endpoint = "#{flow_id}?"
